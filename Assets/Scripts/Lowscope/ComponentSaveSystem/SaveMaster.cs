...
namespace Lowscope.Saving
{
    ...
    public class SaveMaster : MonoBehaviour
    {
        ...
        */
        ...
        {
            var savebles = FindObjectsOfType<Saveable>().ToList();
            if (activeSaveGame != null)
            {
                foreach (var save in activeSaveGame.SaveData)
                {
                    var instance = savebles.FirstOrDefault(s => s.SaveIdentification == save.instanceGuid);
                    if (instance == null)
                    {
                        var prefabSource = ResourcesManager.LoadSourceContainerByPathFromResources(save.prefabPath);
                        if (prefabSource != null)
                        {
                            var prefabInstance = Instantiate(prefabSource.Prefab.gameObject);
                            var s = prefabInstance.GetComponent<Saveable>();
                            s.SaveIdentification = save.instanceGuid;
                            AddListener(s);
                            saveables.Add(s);
                            savebles.Add(s);
                        }
        ...
        public static void SetInt(string key, int value)
        {
            ...
            activeSaveGame.Set(null,null, string.Format("IVar-{0}", key), value.ToString(), "Global");
        }
        ...
        public static void SetFloat(string key, float value)
        {
            ...
            activeSaveGame.Set(null,null, string.Format("FVar-{0}", key), value.ToString(), "Global");
        }
        ...
        public static void SetString(string key, string value)
        {
            ...
            activeSaveGame.Set( null,null,string.Format("SVar-{0}", key), value, "Global");
        }
